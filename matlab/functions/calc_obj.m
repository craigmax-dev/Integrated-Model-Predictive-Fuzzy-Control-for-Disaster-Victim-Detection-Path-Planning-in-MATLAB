...
% V2 REFACTOR V5
function [s_obj, obj] = calc_obj(config, m_f, m_bo_s, m_scan, m_victim_s, dt_s, s_obj, c_f_s, t)
    % Active fires map
    m_fo = (m_f == 3);
    ...
    m_P_fire_weight = m_fo_s * config.weight_fire;

    % First-time scan priority component
    m_P_first_scan = double(m_scan == 0) .* m_victim_s .* (1 + m_P_fire_weight) * config.weight_first_scan;

    % Repeat scan priority component
    time_since_last_scan = max(t - m_scan, 0);  % Time since each cell was last scanned
    m_P_repeat_scan = (m_scan ~= 0) .* time_since_last_scan .* m_victim_s .* (1 + m_P_fire_weight) * config.weight_repeat_scan;
