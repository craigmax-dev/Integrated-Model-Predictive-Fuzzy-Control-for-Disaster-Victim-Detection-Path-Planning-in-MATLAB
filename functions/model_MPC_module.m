...
function [fisArray, ini_params, fis_param_hist] = ...
  ...
  h_MPC = @(params)func_MPC_model(params, ...
    ...
    m_f, m_bo, m_bt, m_s, m_scan, m_t_scan, ...
    dk_a, dk_c, dk_e, dk_mpc, dt_s, k, ...
    n_a, n_p, n_x_s, n_y_s, n_x_f, n_y_f, n_q, ...
    a_loc, a_target, a_task, a_t_trav, a_t_scan, ...
    l_x_s, l_y_s, c_f_s, ...
    c_fs_1, c_fs_2, v_as, v_w, ang_w, ...
    r_bo, r_fo, fis_data);
  ...
  if solver == "fminsearch"
    [mpc_params, ~] = fminsearch(h_MPC, ini_params, fminsearchOptions);
  elseif solver == "ga"
    [mpc_params,~] = ga(h_MPC, nvars, A, b, Aeq, beq, lb, ub, nonlcon, gaOptions);   
  elseif solver == "patternsearch"
    [mpc_params,~] = patternsearch(h_MPC, ini_params, A, b, Aeq, beq, lb, ub, nonlcon, patOptions);   
  elseif solver == "particleswarm"
    [mpc_params,~] = particleswarm(h_MPC, nvars, lb, ub, parOptions);   
  end
